...
/*Paginador*/
/*function envioAjaxSiguiente(){
	inicio=inicio + 10
	$(".borrable").remove();
$.ajax({
    type: "GET",
	url: "selectProveedoresPagina.php",
	contentType: "application/json; charset=utf-8",
	data: {inicio : inicio},
	dataType: "json",
	success: function (result) {
...
//			$('#TablaEmpleado').append(
			var d = '<tr class="clickable-row borrable">'+
			'<td align="center"><button class="btn btn-sm btn-primary btn-click glyphicon glyphicon-pencil margenDerecha" onclick="modificarProveedores()" data-toggle="modal" data-target="#modalEditarProveedor" id="btnEditarUsuario" value="editarProveedor"></button><button class="btn btn-sm btn-danger btn-click glyphicon glyphicon-trash" id="btnEliminarUsuario" value="eliminarProveedor"></button></td>'+
			'<td>'+this.ID_PROV+'</td>'+
			'<td>'+this.NOMBRE+'</td>'+
			'<td>'+this.TELEFONO+'</td>'+
			'<td>'+this.EMAIL+'</td>'+
...
{
	var nombre = $("#nombreProveedorAlta");
	var telefono = $("#telefonoProveedorAlta");
	var email = $("#mailProveedorAlta");
	...
	// Valido Telefono
	var reg = /^[0-9 ]{6,16}$/;
	var match = telefono.val().match(reg);
	if (!match) {
		$("#errorAltaTelefono").siblings().addClass('hidden');
		$("#errorAltaTelefono").removeClass('hidden');
		event.preventDefault();	
		return false;	
	}
	...
	var match = email.val().match(reg);
	if (!match) {
		$("#errorAltaMail").siblings().addClass('hidden');
		$("#errorAltaMail").removeClass('hidden');
		event.preventDefault();	
		return false;
	}
...
function validarProveedoresModificacion()
{
	var nombre = $("#nombreProveedor");
	var telefono = $("#telefonoProveedor");
	var email = $("#emailProveedor");
	...
	var match = telefono.val().match(reg);
	if (!match) {
		$("#errorModificacionTelefono").siblings().addClass('hidden');
		$("#errorModificacionTelefono").removeClass('hidden');
		event.preventDefault();	
		return false;	
	}
	...
	var match = email.val().match(reg);
	if (!match) {
		$("#errorModificacionMail").siblings().addClass('hidden');
		$("#errorModificacionMail").removeClass('hidden');
		event.preventDefault();	
		return false;
	}
